...
45:算指标的有问题？？
46:看DiT的代码
47:做一个因果的sample，至少是从上一层生成下一层 seq-pos
48:添加生成之后返回的代码 create submission adapted
norm_img = (clipped_image - clipped_image.min()) / (
                clipped_image.max() - clipped_image.min()
49:batch_size=16, input_size=256
50:smooth可以提点，也就是说用一个卷积做后处理事可行的
51:更换unet为dm_inpainting 的模型
"""
